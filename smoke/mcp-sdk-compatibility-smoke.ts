...
(async () => {
  ...
  const client = new Client(
    { name: 'vidhya-sdk-compat-test', version: '2.29.0' },
  ...
  try {
    ...
    check(`got 29 tools (admin role)`, toolsResult.tools.length === 29);

    // The SDK enforces that every tool has: name, description, inputSchema.
    // If any of our 29 tools had a misshaped schema, the Zod parse would
    ...
    check(`tool_count matches list (${capsBody.tool_count})`, capsBody.tool_count === 29);
