...
**In 2D**: A rotation by angle $\theta$ is given by the matrix $\begin{pmatrix} \cos\theta & -\sin\theta \\ \sin\theta & \cos\theta \end{pmatrix}$. Watch how the cosine wave below appears to rotate and shift as the parameter $t$ changes—this animation shows the same principle: the transformation evolves smoothly, always respecting the linear structure.
