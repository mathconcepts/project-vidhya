...
Start with two vectors that aren't perpendicular. Trade them for two that are, spanning the same flat space.

Keep the first as-is. From the second, subtract off however much points along the first — what's left is perpendicular to it.

Repeat per new vector, then scale each to length $1$: an orthonormal basis.
