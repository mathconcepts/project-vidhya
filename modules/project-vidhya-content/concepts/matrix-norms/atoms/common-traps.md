...
**Trap 1 — Spectral norm mistaken for spectral radius.** $\|A\|_2=\sigma_{\max}(A)$ is the biggest stretch $A$ applies to any vector. $\rho(A)=\max|\lambda_i(A)|$ only looks at the eigenvalues — the special numbers $\lambda$ solving $Av=\lambda v$. The two match only when $A$ is symmetric — or, more generally, **normal** (meaning $AA^T=A^TA$; every symmetric matrix qualifies, but not every normal matrix is symmetric). $A=\begin{pmatrix}0&1\\0&0\end{pmatrix}$ has $\rho(A)=0$ but $\|A\|_2=1$: a shear that stretches a direction with no eigenvalue attached to it.
