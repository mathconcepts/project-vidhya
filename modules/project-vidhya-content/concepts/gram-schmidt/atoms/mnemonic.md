...
**"Project, then reject."** For each new vector, project it onto every rail already built, then reject (subtract) that projection. What survives is orthogonal (perpendicular) to all of them — that two-word pair is the entire algorithm, repeated once per vector.

**Worked in one line — the same example as the hook:** for $v_2=(2,0)$ against rail $u_1=(1,1)$, project ($c=\tfrac{v_2\cdot u_1}{u_1\cdot u_1}=\tfrac{2}{2}=1$, giving $c\,u_1=(1,1)$), then reject ($u_2=(2,0)-(1,1)=(1,-1)$). Check: $u_1\cdot u_2=1(1)+1(-1)=0$.
