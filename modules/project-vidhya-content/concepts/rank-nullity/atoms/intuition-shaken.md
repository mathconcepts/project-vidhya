...
Same matrix as the animation above, $A=\begin{pmatrix}1&2\\0.5&1\end{pmatrix}$. Watch two specific test vectors go through it.
...
Two things to remember: a direction that lands at $\begin{pmatrix}0\\0\end{pmatrix}$ counts toward **nullity** — check by multiplying it out, nothing fancier. A direction that survives counts toward **rank**. They always add up to the number of columns you started with:

$$\text{rank}(A) + \text{nullity}(A) = n$$

Here, $1+1=2$. **Why it matters for GATE:** rank tells you whether $A\mathbf{x}=\mathbf{b}$ has a solution at all; full rank means invertible; rank-nullity gives the free-variable count immediately.
