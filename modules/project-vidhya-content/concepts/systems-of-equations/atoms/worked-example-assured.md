...
$\text{rank}(A)=\text{rank}([A\mid \mathbf b])=3=n$: unique solution — worth stating before finishing the arithmetic if this is an MCQ asking only for solution count.

Back-substitute: $\boxed{(x,y,z)=(2,3,-1)}$.

**Where marks slip.** Sign errors compound fastest in the last elimination row — recompute the final pivot row independently as a check rather than trusting the running arithmetic. Rank equality alone answers "how many solutions"; full elimination is only needed when the values themselves are wanted.
