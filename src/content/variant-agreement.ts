/**
 * src/content/variant-agreement.ts
 *
 * Pure rules for comparing a stance variant against its base atom.
 *
 * These live in src/ rather than beside the CLI because BOTH the CI gate and
 * the generator need them. A generator that does not enforce the same rules
 * the gate enforces just writes files that fail CI, which is a slower way of
 * finding out the same thing. scripts/check-variant-agreement.ts is now a thin
 * walker over the corpus that calls into here.
 *
...
import { countProseWords, ASSURED_PROSE_BUDGET } from './prose-budget';
