...
Same matrix as the animation above, $C=\begin{pmatrix}1&-1\\-1&1\end{pmatrix}$ — but this time, name the two things it was showing you.
...
**Null space**: the directions the matrix erases — every input $x$ with $Cx=\mathbf{0}$. **Column space**: everything the matrix can actually produce — every $Cx$, landing somewhere on that one surviving line. The rank-nullity theorem ties the two together: their dimensions always add up to the number of columns going in — here, $1+1=2$.
