...
|-------|----------------|--------------------------------------------|
| `marking_table` (per question type) | **Not stated here — see [`src/exams/marking-constants.ts`](../src/exams/marking-constants.ts).** Today's compiled values: MCQ +1/−⅓ or +2/−⅔ · MSQ no negative, no partial unless verified · NAT no negative | MCQ +4/−1 · numeric +4/0 — confirm current NTA scheme at authoring time |
...
- **`marking_table`'s numbers live in exactly one file, and it is not this one.** [`src/exams/marking-constants.ts`](../src/exams/marking-constants.ts) is the compiled marking truth (plan D7/E6). The GATE-EM cell above is a prose rendering of it, re-pointed rather than restated — the same treatment applied to the four code sites that used to state these fractions independently (`deterministic-scorer.ts`'s defaults, `exam-profile.ts`'s `GATE_EM_MARKING_TABLE`, `exam-catalog.ts`'s `marking_table`, `samples/gate-mathematics.ts`'s `marking_scheme`), all of which now derive from it. The `assessment_contracts` seed row (`supabase/migrations/050_assessment_contracts.sql`) is generated from the same constant and held to it by a byte-equality test. **When a marking scheme changes, change that module — never this table, and never one of the four derived sites.**
