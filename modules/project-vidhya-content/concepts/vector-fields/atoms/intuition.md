...
Take $\phi(x,y) = \tfrac12(x^2+y^2)$, a bowl with its bottom at the origin. The gradient is $\nabla\phi=(x,y)$ — which is exactly $\mathbf F(x,y)=(x,y)$, the field from the hook. So that field was a gradient all along: at $(1,1)$, $\nabla\phi=(1,1)$, an arrow pointing away from the origin — exactly the uphill direction on this bowl, since height only increases as you move outward. At $(0,0)$, the bottom, $\nabla\phi=(0,0)$: flat ground, no uphill direction at all.
