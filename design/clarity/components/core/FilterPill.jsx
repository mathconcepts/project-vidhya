...
export function FilterPill({ active = false, count, onClick, children }) {
  return (
    <button type="button" onClick={onClick} style={{
      ...
      fontFamily: 'var(--font-sans)', fontSize: 'var(--text-footnote)', fontWeight: 'var(--weight-medium)',
      whiteSpace: 'nowrap', transition: 'background var(--dur-fast) var(--ease-standard)',
    }}>
      {children}
      {count != null && <span style={{ opacity: 0.55, fontVariantNumeric: 'tabular-nums' }}>{count}</span>}
    </button>
  );
}
