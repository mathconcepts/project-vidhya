...
**GATE Problem: Joint PDF on a Triangular Region.** Let $(X, Y)$ have joint PDF:
...
$$E[X] = \int_0^1 x \cdot 2(1-x)\,dx = 2\left[\frac{x^2}{2} - \frac{x^3}{3}\right]_0^1 = 2\left(\frac{1}{2} - \frac{1}{3}\right) = \frac{1}{3}$$

$$E[Y] = \int_0^1 y \cdot 2y\,dy = 2 \cdot \frac{1}{3} = \frac{2}{3}$$

Sanity check: $E[X] < E[Y]$ is expected since $X < Y$ always.

## Step 4 — Compute $E[XY]$

$$E[XY] = \int_0^1 \int_0^y 2xy\,dx\,dy = \int_0^1 2y \cdot \frac{x^2}{2}\bigg|_0^y dy = \int_0^1 y^3\,dy = \frac{1}{4}$$

## Step 5 — Covariance

$$\text{Cov}(X,Y) = E[XY] - E[X]\,E[Y] = \frac{1}{4} - \frac{1}{3} \cdot \frac{2}{3} = \frac{9}{36} - \frac{8}{36} = \boxed{\frac{1}{36}}$$
...
Reach for this direct double-integral setup whenever the joint support is a described region (here, the triangle $0<x<y<1$) rather than a product of independent intervals — the tempting shortcut is to integrate $x$ from $0$ to $1$ and $y$ from $0$ to $1$ as if the variables were independent, which silently double-counts probability outside the true triangular support and breaks the "integrates to 1" check in Step 1.
