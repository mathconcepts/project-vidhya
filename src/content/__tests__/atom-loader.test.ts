 ...
 * The 3 seed concepts (derivatives-basic, complex-numbers,
 * eigenvalues) all have atoms/ folders post-Phase 1.
 */
...
describe('loadConceptAtoms — fallback chain', () => {
  it('loads atoms when atoms/ folder exists', async () => {
    const atoms = await loadConceptAtoms('derivatives-basic');
  ...
  it('parses YAML frontmatter into typed fields', async () => {
    ...
    expect(wp!.id).toContain('derivatives-basic');
  ...
  it('caches results — second call returns same array reference', async () => {
    const a1 = await loadConceptAtoms('derivatives-basic');
    const a2 = await loadConceptAtoms('derivatives-basic');
    expect(a1).toBe(a2);
  });
...
describe('loadConceptMeta — additive schema', () => {
  it('loads existing v1 fields (title, exams, tags)', async () => {
    const meta = await loadConceptMeta('derivatives-basic');
    expect(meta.concept_id).toBe('derivatives-basic');
