...
On $A=\begin{pmatrix}1&1\\1&-1\end{pmatrix}$, the hook's own matrix: watch why the crossing point never disappeared, for any $t$, below.
...
A system is $A\mathbf{x}=\mathbf{b}$. A solution exists when $\text{rank}(A)=\text{rank}([A\mid\mathbf{b}])$ — the *augmented matrix*, $A$ with column $\mathbf{b}$ stuck on beside it.

| Ranks | Solutions |
|---|---|
| unequal | zero |
| equal, $=n$ | exactly one |
| equal, $<n$ | infinitely many |

Two ways to solve: row-reduce and back-substitute, or Cramer's rule for small square systems.
