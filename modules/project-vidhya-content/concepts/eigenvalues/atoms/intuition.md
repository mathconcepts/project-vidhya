...
Picture a matrix $A$ as a transformation. You feed it a vector $v$, it spits out $Av$. For most $v$, the output points in a different direction — but for a few special $v$, it doesn't. Try three vectors on the hook's own matrix and see which ones qualify.
...
Eigenvectors are the axes the matrix doesn't rotate. They're the natural coordinate system *of that matrix*. Drag a different matrix below and watch which axes stay put change with it.
