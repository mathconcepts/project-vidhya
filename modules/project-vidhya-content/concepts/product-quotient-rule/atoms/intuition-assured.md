...
The condition that makes this valid: $\ln|f|$ requires $f\neq0$ on the interval in question, and the absolute value is load-bearing, not decoration — $\ln f$ alone is undefined wherever $f<0$, even though $\dfrac{d}{dx}\ln|f|=\dfrac{f'}{f}$ holds on both sides of a sign change.
