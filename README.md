...
| 🎬 **Interactives + multi-modal sidecars** | Three dependency-free interactive atom kinds (Manipulable / Simulation / GuidedWalkthrough) ride next to atom bodies. Simulation scenes add a manual drag-to-scrub slider alongside play/pause, for beat and non-beat scenes alike. GIF scenes render via `gifenc` (sync, pure-JS); TTS narration via OpenAI tts-1, A/B-gated and cost-capped. `prefers-reduced-motion` honoured. | `frontend/src/components/lesson/interactives/`, `src/content/concept-orchestrator/{gif,tts}-generator.ts` |
| ✨ **Resonance beats** | A hook or intuition atom's prose, motion and one moment-of-need mistake fuse into a single scripted scene instead of three siloed cards — it autoplays once on mount, the caption advances in step with the trace, and the one point students actually slip draws in as a dashed grey "ghost path" that stays on screen with its fix for the rest of playback. Shaky and confident students see the same true geometry with different per-stance captions. Reduced motion gets the full argument as static text, nothing lost. Live on 24 of 26 GATE Linear Algebra concepts + 2 non-LA proof scenes; generation-side prompt wiring carries it forward for every future batch. | `frontend/src/components/lesson/interactives/Simulation.tsx`, `src/content/resonance-strategy.ts`, `docs/designs/2026-08-30-resonance-fused-atoms-plan.md` |
