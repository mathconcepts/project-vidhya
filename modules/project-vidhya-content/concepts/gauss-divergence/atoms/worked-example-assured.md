...
The mark at risk isn't the arithmetic, it's the hypothesis check: this shortcut needs $\mathbf F$ differentiable everywhere inside $V$, not merely on $S$. A field like $\hat r/r^2$ fails that hypothesis right at the singularity, and constant-times-volume stops being valid the instant one sits inside the surface — confirm smoothness on the interior before reaching for the volume formula, not after.
