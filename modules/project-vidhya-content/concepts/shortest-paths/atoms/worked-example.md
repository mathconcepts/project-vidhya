...
```interactive-spec
{"v":1,"kind":"guided_walkthrough","steps":[{"prompt":"After settling vertex A (distance 0), what are the updated distances d[B] and d[C]? Which vertex is extracted next?","hint":"Relax A's outgoing edges: A→B (weight 4) and A→C (weight 2). The vertex with minimum distance is extracted next.","answer":"d[B] = 4 (via A→B), d[C] = 2 (via A→C). C is extracted next because d[C]=2 < d[B]=4."},{"prompt":"After settling C (distance 2), d[B] improves. State the new d[B] and explain why the initial value of 4 is replaced.","hint":"Relax C's edge to B: d[C] + w(C,B) = 2 + 1 = 3. Compare with the current d[B] = 4.","answer":"New d[B] = 3 (via A→C→B, cost 2+1=3). Since 3 < 4, we relax: d[B]=3 and predecessor π[B] is updated to C. This is the fundamental relaxation step of Dijkstra."}]}
```
