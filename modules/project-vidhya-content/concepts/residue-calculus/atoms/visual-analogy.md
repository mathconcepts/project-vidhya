...
The residue theorem lets a contour shrink to the smallest loop that still isolates a single pole — the value of the integral doesn't change as long as no other singularity crosses the boundary on the way in. The diagram on this card shows exactly that: a tiny loop drawn around one isolated pole, small enough to enclose nothing else.
