...
**Step 1 — Characteristic polynomial.** $\det(A-\lambda I)=(4-\lambda)(2-\lambda)-(1)(-1)=\lambda^2-6\lambda+9=(\lambda-3)^2$. Eigenvalue $\lambda=3$, algebraic multiplicity $2$ (it shows up twice as a root).

---

**Step 2 — Eigenspace.** $A-3I=\begin{pmatrix}1&1\\-1&-1\end{pmatrix}$, rank $1$ (only one independent row), so $\dim\ker(A-3I)=2-1=1$. Geometric multiplicity $1<2$: only one independent eigenvector exists, fewer than the repeat count — the matrix is **defective** (too few independent eigenvectors to diagonalize). Solving $(A-3I)v=0$: $v_1+v_2=0$, so $v=(1,-1)$.

---

**Step 3 — Generalized eigenvector.** Geometric multiplicity $1$ with algebraic multiplicity $2$ forces a single Jordan block of size $2$: one more chain vector — a **generalized eigenvector**, a stand-in that fills the gap left by the missing genuine eigenvector — is needed. Solve $(A-3I)w=v$: $w_1+w_2=1$, so $w=(1,0)$ works.
