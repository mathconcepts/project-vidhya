...
id: numerical-integration.worked-example.assured
...
$n=4$ is even, satisfying Simpson's structural requirement, and $f(x)=(1+x)^{-1}$ has $f^{(4)}(x)=\frac{24}{(1+x)^5}$ continuous on $[0,1]$, maximized at $x=0$ where it equals $24$ — so the $O(h^4)$ bound genuinely applies here.

$$|E|\le\frac{(1)(0.25)^4}{180}(24)\approx0.00052$$

$$\text{Simpson},\,n{=}4:\ 0.693254\qquad \ln2\approx0.693147\qquad|E|=0.000107<0.00052$$

The trapezoidal rule at the same $h$ — only $O(h^2)$ — gives an error near $0.0039$, roughly twenty times worse for the same number of evaluations.
