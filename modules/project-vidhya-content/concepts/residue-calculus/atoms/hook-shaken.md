...
Take $\oint_{|z|=2}\frac{dz}{z(z-1)}$: two poles inside, residues $-1$ at $z=0$ and $1$ at $z=1$ — found in seconds, not by tracing the whole loop. Sum them: $0$. Multiply by $2\pi i$: the integral is $0$, the whole loop reduced to two numbers. Below, watch that same loop trap both poles at once.
