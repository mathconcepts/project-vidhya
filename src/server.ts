...
import { explanationFrameRoutes } from './api/explanation-frame-routes';
...
import { graphRoutes } from './api/admin-graph-routes';
...
for (const route of conceptResolveRoutes) {
  registerRoute(route.method, route.path, route.handler);
}
...
for (const route of adminDecisionsRoutes) {
  registerRoute(route.method, route.path, route.handler);
}
for (const route of adminCohortRoutes) {
  registerRoute(route.method, route.path, route.handler);
}
...
for (const route of explanationFrameRoutes) {
  registerRoute(route.method, route.path, route.handler);
}
