...
The curve on this card traces $y=\det(A_t)$ for $A_t=\begin{pmatrix}1&t\\1&1\end{pmatrix}$ as $t$ moves from $-2$ to $2$: $\det(A_t) = 1-t$. At $t=1$, the determinant hits zero — the two rows become identical, $\text{rank}$ drops from $2$ to $1$, and a whole new direction opens up in the null space that didn't exist a moment before. Away from $t=1$, both rows are independent and $\text{nullity}=0$.
