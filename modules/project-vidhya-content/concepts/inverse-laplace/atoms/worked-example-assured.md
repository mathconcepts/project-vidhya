...
$\dfrac{s+2}{(s+1)(s^2+4)}$ splits as $\dfrac{1/5}{s+1}+\dfrac{-s/5+6/5}{s^2+4}$ — cover-up at $s=-1$ for $A$, coefficient matching for $B,C$ — skip re-deriving once cover-up is automatic.

$$f(t)=\frac15e^{-t}-\frac15\cos 2t+\frac35\sin 2t,\quad t\geq0$$

The step worth double-checking isn't the decomposition, it's the rescale after it: $\dfrac{6/5}{s^2+4}$ is not $\dfrac{6}{5}\sin2t$. The sine pair needs $\omega$ sitting in the numerator, so pull it out first — $\frac35\cdot\frac{2}{s^2+4}\to\frac35\sin2t$. Forgetting that rescale is the error that survives a fully correct partial-fraction step and still costs the mark.

Pole check: $s=-1$ real and stable, $s=\pm2j$ an undamped pair — every pole has non-positive real part, so $f(t)$ must stay bounded. A pole with positive real part surviving to this stage means an earlier sign error, not a legitimately growing answer.
