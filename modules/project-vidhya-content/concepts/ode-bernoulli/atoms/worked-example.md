...
```interactive-spec
{"v":1,"kind":"guided_walkthrough","steps":[{"prompt":"For $\\\\dfrac{dy}{dx} - y = xy^2$, identify $n$, $P(x)$, and $Q(x)$.","hint":"Write in standard form $y' + P(x)y = Q(x)y^n$ and match each part.","answer":"$n = 2$, $P(x) = -1$, $Q(x) = x$. The equation is $y' + (-1)y = x \\\\cdot y^2$."},{"prompt":"What substitution do you make, and how does $y^{-2}\\\\dfrac{dy}{dx}$ transform?","hint":"Set $v = y^{1-n} = y^{-1}$. Differentiate both sides with respect to $x$.","answer":"$v = y^{-1}$, so $\\\\frac{dv}{dx} = -y^{-2}\\\\frac{dy}{dx}$, meaning $y^{-2}\\\\frac{dy}{dx} = -\\\\frac{dv}{dx}$. The Bernoulli becomes $\\\\frac{dv}{dx} + v = -x$."},{"prompt":"After solving for $v = 1 - x + Ce^{-x}$, write the final answer for $y(x)$ and then find $C$ if $y(0) = 1$.","hint":"Recall $v = 1/y$, so $y = 1/v$. Substitute $x=0$, $y=1$ into the general solution.","answer":"$y(x) = \\\\dfrac{1}{1 - x + Ce^{-x}}$. With $y(0)=1$: $1 = 1/(1+C)$, so $C = 0$ and $y = \\\\dfrac{1}{1-x}$."}]}
```
