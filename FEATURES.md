...
## Slide 33 — What's Shipped (at v2.33.0)
...
| v2.26.0 | `cf55c7d` | MCP prompts/ primitive + dashboard relocated to same-origin static. (1) MCP prompts — third primitive after tools + resources, with prompts/list + prompts/get JSON-RPC methods; 6 role-scoped prompt templates (daily-standup, triage-briefing, strategy-review, task-handoff, week-in-review, content-debt-report) that return MCP-formatted messages the CLIENT runs through its own LLM; capabilities.prompts declared with listChanged=false; error codes -32001 prompt-not-found, -32002 not-authorized, -32602 invalid-arguments, -32603 internal; argument validation for required params; all prompt builders deterministic (no server-side LLM calls). (2) Dashboard moved from /api/admin/agent/dashboard to /admin/agent/dashboard — now served from frontend/dist/admin/agent/dashboard/index.html, lives alongside the student-facing SPA under the same origin + CSP, no longer pollutes the /api/ namespace; canonical source remains src/admin-orchestrator/dashboard-html.ts with a new scripts/regenerate-dashboard-static.ts that writes the TS template output to frontend/public (picked up by Vite) AND frontend/dist (for immediate dev) — single source of truth, one committed artifact. (3) server.ts static handler patched to serve index.html from directories matching /foo/ paths (standard static-host behavior). (4) Old /api/admin/agent/dashboard now returns HTTP 301 to /admin/agent/dashboard preserving any existing bookmarks. 97/97 smoke cases pass across 18 case groups. |
| v2.27.0 | `bcaaabe` | MCP logging/setLevel + Prompts tab in dashboard + integration docs. (1) Central logger (src/admin-orchestrator/logger.ts, ~220 LOC) with 8 MCP-standard levels (debug/info/notice/warning/error/critical/alert/emergency), bounded 200-event ring buffer, subscribe/unsubscribe API, per-session level tracking, stderr passthrough for operator visibility. (2) MCP logging/setLevel method — session-keyed level filter; under stdio transport, logger subscriptions push notifications/message to stdout matching MCP spec (jsonrpc 2.0 + method + params.{level,logger,data}); under HTTP transport, accepts setLevel for compliance then exposes ring buffer via new resource vidhya://admin/logs/recent with session-threshold applied. stdio-server.ts refactored to use central logger throughout (no more local log helper). (3) Prompts tab in dashboard — third tab in MCP Explorer showing all 6 prompts, with per-prompt argument inputs (required params marked with red asterisk), Generate button that calls prompts/get via JSON-RPC, on-success preview in a collapsible details element with character count, Copy to Clipboard button using navigator.clipboard with execCommand fallback for non-HTTPS contexts, and green 'ok' toast on successful copy. (4) Documentation — docs/mcp-integration.md (320 lines) with copy-paste Claude Desktop + Cursor + OpenAI Python configs, HTTP curl examples for all 4 primitives, environment variable reference, surface catalog, error code table, troubleshooting section. docs/admin-dashboard-quickstart.md (160 lines) — 5-step install-to-connected walkthrough. 118/118 smoke cases pass across 17 case groups. |
| v2.28.0 | `c0a7849` | MCP completion/complete + OpenAPI 3.1 export + stdio subprocess integration test. (1) MCP completion/complete — argument auto-complete for prompt arguments (task_id, week_start, priority_filter, etc.) and resource template placeholders (URIs like vidhya://admin/tasks/{task_id}). Resolvers centralized by argument name (10 supported: task_id, run_id, strategy_id, feedback_id, exam_id, role, priority_filter, priority, week_start, build_kind) rather than per-prompt so the same logic serves every reference that uses that name. Ranking: exact > prefix > substring > alphabetical. Soft cap 50 values per call with hasMore + total. Validation pipeline: ref.type shape (-32602), argument.name shape (-32602), argument declared on referenced prompt/resource (-32001 if not), unknown argument returns empty-without-error. Capabilities now declares completions:{}. (2) OpenAPI 3.1 export at GET /api/admin/agent/openapi.json — public endpoint generating full spec at request time describing all 20 admin-agent routes. OpenAPI 3.1 chosen over 3.0 because it aligns with JSON Schema Draft 2020-12 (our tool input_schemas lift straight through). 6 tags (Runs/Tasks/Tools/MCP/Diagnostics/Dashboard), bearerAuth security scheme (JWT), component schemas for RoleId/Priority/TaskStatus/HealthReport/Signal/Strategy/Task/AgentRun/JsonRpcRequest/Success/Error. /mcp path documents 6 example request bodies (initialize, toolsList, toolsCall, resourcesRead, promptsGet, completionComplete). x-vidhya-stats extension surfaces runtime tool_count/resource_count/prompt_count/generated_at for discoverability. (3) stdio subprocess integration test (smoke/stdio-integration-smoke.ts, ~300 LOC) — spawns real npx tsx src/admin-orchestrator/stdio-server.ts process, writes newline-delimited JSON-RPC to stdin, parses responses + notifications from stdout through a line-buffered StdioClient class with 10s per-request timeout. 12 test cases cover: subprocess startup + liveness, initialize handshake, ping, tools/list (29), tools/call round-trip, resources/list (11), prompts/list (6), completion/complete, logging/setLevel + notifications/message push verification (before=2/after=2 confirmed filter works mid-session), malformed JSON on stdin produces -32700 with id=null without crashing process, subprocess survives bad input, clean exit code 0 on stdin close. 49/49 end-to-end over real pipes plus 104/104 offline smoke across 15 case groups. |
| v2.29.0 | `0612316` | Swagger UI at /admin/agent/docs + MCP SDK compatibility test. (1) New docs page at /admin/agent/docs — single-file static HTML (src/admin-orchestrator/docs-html.ts, 9KB) rendering Swagger UI v5.17.14 (pinned, OpenAPI 3.1 capable) over the live /api/admin/agent/openapi.json spec. Dark theme override matching the dashboard, CDN-failure fallback pointing at the raw spec URL, tryItOutEnabled + persistAuthorization so one-time JWT paste authenticates every Try-it-out. Served via the same same-origin static-file pattern as the dashboard (no /api/ prefix, no server route) — existing server directory-with-index.html handler (v2.26) serves it without changes. Regenerate script extended to manage both artifacts (dashboard + docs) side by side. (2) @modelcontextprotocol/sdk v1.29.0 added as devDependency for a new end-to-end compatibility test (smoke/mcp-sdk-compatibility-smoke.ts, 320 LOC) — spawns our stdio server under Anthropic's official SDK's Client + StdioClientTransport, exercises every one of the 10 protocol methods (ping, tools/list, tools/call, resources/list, resources/read, prompts/list, prompts/get, complete, setLoggingLevel, notifications/message push), and validates every response shape against the SDK's Zod schemas. The SDK's schemas are the reference implementation of what Claude Desktop, Cursor, and the MCP Inspector expect — passing them is the strongest possible 'spec-correct' signal. 65 assertions all pass across 12 case groups, including LoggingMessageNotificationSchema validation of pushed events and mid-session setLoggingLevel filtering (before=2 after=2). Combined smoke: 63 (v2.29 offline) + 65 (SDK compat) + 49 (stdio integration) = 177 total assertions, all pass. |
| v2.30.0 | `8714e40` | Pivot to student-facing work — session planner. First of the student-side primitives, ties together three existing pure modules (priority engine, attention resolver, content-hint shape) into a single 'given my current state + time + exam, what should I do in the next N minutes?' API. (1) src/session-planner/planner.ts (~260 LOC) — pure function from PlanRequest to SessionPlan; composes attention/resolver.budgetFromMinutes + resolveStrategy (nano/short/medium/long classification, gbrain_max_recommendations, mock_difficulty_mix, gbrain_bias) with engine/priority-engine.computePriority (marks_weight × weakness × improvement_speed × recency_due × exam_proximity) into an ordered list of ActionRecommendations. Each action carries kind (practice/review/spaced-review/micro-mock), rationale, estimated_minutes, content_hint (topic+difficulty+count for the frontend resolver), priority_score. Invariants: sum(estimated_minutes) ≤ budget, ≥1 action always, deterministic (same inputs → same plan structure), no I/O in core. Strategy gbrain_bias drives topic ranking: quick_win prefers 0.55-0.75 accuracy topics (closable gaps), prerequisite_repair prefers <0.45 (deep foundation), balanced is raw priority order. Spaced-review auto-surfaces for topics last practiced ≥3 days ago with <60% accuracy. Long sessions (>25min) get a micro-mock capstone when ≥8min remain after main practice. (2) src/session-planner/store.ts (~70 LOC) — flat-file store at .data/session-plans.json, append-only with lazy pruning (50 plans per student_id max), matching admin-orchestrator store conventions. (3) src/api/session-planner-routes.ts (~130 LOC) — POST /api/student/session/plan + GET /api/student/session/plans + GET /api/student/session/plans/:id, all auth-required, student_id forced from authenticated JWT (request body's student_id is ignored to prevent cross-user scope leaks). (4) docs/session-planner.md (~170 lines) — request/response examples for short/nano/long session plans, invariant statement, separation-of-concerns explanation (planner hints vs content resolver vs gbrain). Smoke: 65 assertions across 15 case groups covering determinism, budget respect, spaced-review surfacing, nano lead-with-review behavior, long-session micro-mock, exam-proximity effect on priorities, persistence round-trip, 50-plan pruning cap, HTTP 401 on missing auth. Also refreshed v2.28 stdio-integration smoke to use regex version assertion (was hardcoded 2.28.0). |
| v2.31.0 | `8467cab` | Complete all 6 student-facing next steps from v2.30 retro in one release. (1) Multi-exam planner — planMultiExamSession() runs the single-exam planner for each exam with minutes allocated proportional to an exam-proximity weight (30/(30+daysRemaining) hyperbolic curve; 1-day-out ≈ 1.0, 30-day ≈ 0.5, 180-day ≈ 0.14), then interleaves + sorts actions globally by priority_score. 2-min-per-exam floor with fallback to closest-exam-only plan if every exam falls below floor. Supports 1-5 concurrent exams. Actions tagged with exam_id for UI grouping. (2) Completion endpoint at POST /api/student/session/plans/:id/complete — PlanExecution recorded inline on the plan (completed_at, actual_minutes_spent, actions_completed: [{action_id, completed, attempts?, correct?, actual_minutes?, note?}]). Idempotent overwrite (latest self-report wins). Scope-checked 404/403 on unknown plan / other-student's plan. Validation rejects correct > attempts. (3) Trailing-7d-minutes derived server-side — sumTrailingMinutes(student_id, days, now) sums actual_minutes_spent across plans where execution.completed_at falls within the window. Plan request layer uses this when client omits trailing_7d_minutes. Compounding-short-session recognition (v2.30 attention primitive) now works automatically without self-report. (4) Projected sr_stats via projectSrStatsFromExecutions(student_id) — aggregates per-topic (attempts, correct, first-5 vs last-5, last_practice_date) from completed plan outcomes. Bootstrap path: when client provides no sr_stats, plan route uses projection so planner self-improves over time without a separate gbrain store. (5) JEE Main Mathematics adapter (EXM-JEEMAIN-MATH-SAMPLE) — third bundled exam (BITSAT + UGEE + JEE). Real NTA 2026 spec: 30 questions (20 MCQ + 10 NAT), +4/-1 on MCQ, +4/-0 on NAT, 60-min math section, calculus-weighted priority_concepts vs BITSAT's algebra lead. defaultGenerationSections() uses 2:1 mock_question:nat_question ratio matching the 20:10 exam mix. 240-LOC sample (vs BITSAT's 900) — relies on shared lesson bank rather than duplicating per-exam lessons. (6) 3 MCP tools for the planner — student:plan-session (category=analysis, required_roles owner/admin/analyst), student:list-plans (read), student:get-plan (read). Analytics-style: take student_id as parameter (not forced from JWT) since MCP clients are admin/analyst operators querying on behalf of students. Tool count 29 → 32. Input schemas (Draft 2020-12): plan-session requires student_id/exam_id/exam_date/minutes_available with proper enums (minimum:1/maximum:180 on minutes, format:date on exam_date, enum on difficulty), get-plan enforces ^PLN-[a-z0-9]+$ pattern. author role correctly denied on plan-session. Frontend: PlannedSessionPage at /gate/planned (420 LOC) — 5 preset buttons (3/8/15/30/60 min) + range slider for fine control, authFetch against /api/student/session/plan, action cards with kind-meta icons (BookOpen/RefreshCw/AlertCircle/Sparkles) + per-action rationale + estimated minutes + Start/Mark-done/Skip controls. Start navigates to /gate/smart-practice with topic+difficulty+from_plan+action_id query params (existing page already handles these). Per-action attempts/correct inputs for outcome tracking. Finish button posts PlanExecution with real elapsed minutes (measured from plan generation to finish click). Lazy-loaded in App.tsx, Route registered after smart-practice. Routes: 5 total (was 3) — added plan/multi-exam and plans/:id/complete. Tools: 32 total (was 29). Adapters: 3 total (was 2). Smoke: 70 v2.31 offline + 65 SDK compat + 49 stdio = 184 total assertions, all pass. Regression fixes: SDK compat smoke tool count 29→32, stdio integration smoke tool count 29→32. No breaking changes. |
| v2.32.0 | `c445b60` | Complete all 7 next steps from v2.31 retro in one release. (1) Multi-exam setup UI — ExamProfilePage at /gate/exam-profile (280 LOC) lets students register 1-5 concurrent exams with dates, pre-selecting from known exam ids (UGEE/BITSAT/JEE Main) or custom strings. Whole-profile PUT via /api/student/profile; flat-file-store at .data/student-exam-profiles.json. PlannedSessionPage reads the profile on mount and auto-routes: 0 exams → default fallback, 1 exam → single-exam planner, ≥2 → multi-exam planner. Inline 'use your profile' nudge when empty. (2) student:get-plan-with-execution MCP tool — combines plan fetch with outcome rollup in one call (total_attempts, total_correct, accuracy, minutes_planned vs minutes_actual, adherence_ratio, per_topic breakdown). Coaching tools no longer stitch two separate calls. Tool count 32 → 33. (3) Student-facing Swagger UI at /student/docs — OpenAPI 3.1 generator (src/api/student-openapi.ts, 390 LOC) describes all 13 session-planner routes with schemas + examples. Dark-theme Swagger UI v5.17.14 template (src/session-planner/docs-html.ts) pinned for reproducibility. Served via the existing same-origin static-file pattern (no new server route). Regenerate script extended — now produces 3 artifacts (admin dashboard + admin docs + student docs) × 2 targets each = 6 outputs. (4) Real-world trailing-minutes integration — src/session-planner/practice-session-log.ts (130 LOC) stores timestamped per-session entries with source tagging (smart-practice, practice-page, mock-exam, plan-execution, other). sumTrailingMinutes() unions plan executions + practice log, skipping entries tagged with plan_id to prevent double-counting (belt-and-braces safety). POST /api/student/session/practice-log endpoint for ad-hoc client logging. h_complete mirrors each plan execution into the practice log for consistency. 30-day lazy pruning. Became async; 3 call sites in routes updated with await. (5) Live LLM smoke harness — discovered existing smoke/live-llm-smoke.ts (279 LOC) already ships comprehensive test across all 3 LLM-backed tools (narrate-strategy, summarize-health, suggest-next-action) against anthropic/google-gemini/openai providers. Added npm scripts: smoke:stdio, smoke:sdk-compat, smoke:live-llm, smoke:all. Skip path verified (no-creds returns exit 0 with clear 'SKIPPED (no LLM configured)' message). (6) Admin dashboard student-plans panel — renderRecentStudentPlans() in dashboard-html.ts adds a collapsible section with refresh button, compact roll-up per plan (id, student, action count, minutes planned/actual, completion ratio, timestamp). Uses student:list-plans MCP tool with '*' wildcard; dispatch extended to accept wildcard → listAllPlans(limit). Role-gated at the tool level (analyst+). (7) Plan templates — src/session-planner/template-store.ts (120 LOC) with 20-per-student cap, use_count tracking for most-used-first sort. 4 HTTP routes: list/create/delete/use. POST /templates/:id/use is the one-tap recall path — loads student's profile, picks single vs multi-exam planner, fires plan, returns {template_id, plan}. Template bar in PlannedSessionPage with click-to-recall, hover-to-delete. Inline save-as-template with purple-tinted form. Routes: 13 total (was 5). MCP tools: 33 total (was 32). Static artifacts: 3 (was 2). Smoke: 74 v2.32 offline + 65 SDK compat + 49 stdio = 188 total assertions, all pass. Live LLM smoke harness present but skipped in no-creds CI. Regressions: SDK compat + stdio tool-count 32→33. No breaking changes. |
| v2.33.0 | *this* | Complete all 6 natural next steps from v2.32 retro in one release. (1) Attention store unification — recordSession became async, writes through the timestamped practice log. getCoverage now derives trailing_7d_minutes by reading the log synchronously via fs.readFileSync (no async propagation to readers), with fallback to the legacy inline counter when the log file is missing. Cross-store union verified: attention writes + SP.logPracticeSession writes both visible from both surfaces. (2) Dashboard filter by student — text input on the Recent Student Plans panel with Enter-to-refresh. Empty input → '*' wildcard (v2.32 behaviour preserved); populated → passes student_id to the tool. Clear-filter button appears when filter is active. (3) Template presets — src/session-planner/template-presets.ts ships a curated catalog of 5 starter templates (bus-stop/commute/morning/afternoon/weekend) with minutes 3/8/15/30/60. New GET /api/student/session/templates/presets returns the catalog + per-preset adopted flag. PlanTemplate shape extended with optional preset_slug field; POST /templates accepts it; h_listPresets uses it to filter. PlannedSessionPage renders unadopted presets as dotted-border suggestion cards. Adoption is one-click: POST template with preset_slug + immediately call /use to recall. (4) Trailing-minutes badge — new GET /api/student/session/trailing-stats endpoint returns {trailing_7d_minutes, trailing_7d_sessions, computed_at}. PlannedSessionPage fetches on mount alongside profile/templates/presets and displays an emerald-tinted 'You've studied N min across M sessions this week' badge when N > 0. (5) Live LLM smoke — cannot run in this sandbox (no API credentials). Harness at smoke/live-llm-smoke.ts remains ready; npm run smoke:live-llm wired; skip path continues to return exit 0 with 'SKIPPED (no LLM configured)' message. Remains outstanding as a manual step when credentials are available. (6) Pre-existing frontend type errors — shipped frontend/src/vite-env.d.ts with vite/client reference (fixes 4 ImportMeta.env errors). Fixed 11 implicit-any callback parameters in ExamSetupPage.tsx. Fixed void-truthiness test in GiveawayBanner.tsx by splitting the onClick into two statements. Fixed genuine CountUp API mismatch in ExamStrategyPage.tsx by removing the non-existent `decimals` prop and pre-rounding the value client-side. Frontend typecheck now zero errors (was 17). Routes: 15 total (was 13: added /trailing-stats + /templates/presets). MCP tools: 33 (unchanged — no new MCP tools this release). Smoke: 49 v2.33 offline + 65 SDK compat + 49 stdio = 163 assertions, all pass. Frontend: zero TypeScript errors. Backward-compatible: recordSession signature unchanged from callers' POV (was non-awaited void-effect, now awaited Promise — single caller in attention-routes.ts updated). No breaking MCP surface changes. |
| post-v2.33 org refresh | *structural — no feature changes* | Structural reorganisation of the codebase around a 48-agent one-person-company graph. No src/ functionality changed; no smoke-test assertions changed. (1) NEW `agents/` directory — 46 agent manifests defining a four-tier org graph (1 CEO + 6 C-suite + 17 managers + 22 specialists → later 24 specialists after GBrain wiring). Manifests are runtime-agnostic YAML; communication runs through a five-channel envelope (delegation / query / escalation / signal / response). Graph validator at agents/validate-graph.py enforces 8 invariants: reports-to resolution, connection-id resolution, no cycles, exactly one root (CEO), ≤8 downstreams per agent, every MCP tool referenced exists, and the GBrain cognitive-spine invariant. (2) GBrain wiring — the cognitive-spine contract at agents/_shared/gbrain-integration.md names every agent that reads or writes GBrain and maps the 8 GBrain modules (student-model / error-taxonomy / task-reasoner / problem-generator / exam-strategy / after-each-attempt / integration / cross-exam-coverage) to specific agents. 11 agents declare src/gbrain/* dependencies. Validator check 7 enforces the cognitively-dependent set — cdo, student-model-manager, planner-manager, teaching-manager, assessment-manager, authoring-manager, feedback-manager — must each declare at least one src/gbrain/* module. Two new GBrain specialists added: problem-generator-specialist (teaching) and attempt-insight-specialist (the canonical write path under assessment). (3) Event-bus type shim — src/events/event-bus.ts added with minimal EventBus + EventBusEvent interfaces. Fixes a latent bug where src/channels/service.ts imported EventBus from a non-existent path; the import survived only because of TypeScript type-elision (type-only use of the name, erased at runtime by esbuild). The shim makes the contract textual. Type-only, no runtime behaviour added. (4) Vitest unit suite cleanup — removed 8 stale test files from v2.2.3 that referenced modules renamed (src/orchestrator → src/admin-orchestrator), removed (SageAgent/OracleAgent/MentorAgent persona classes), or with evolved APIs (Cache → InMemoryCache/CacheManager/NamespacedCache; LLMClient constructor signature changed). Suite now 3 files / 107 tests / all passing (was 12 files / 163 run / 53 failing with imports to nonexistent modules). (5) Software verification matrix — full pass on six gates: backend typecheck 0 errors, frontend typecheck 0 errors, vitest 107/107, stdio smoke 49/49, SDK compat smoke 65/65, agent graph valid. 270 total verified assertions. Live-LLM smoke remains the seventh manual gate (requires API keys). (6) Documentation reflects current state — docs/00-index.md and docs/02-agent-architecture.md carry legacy-architecture banners pointing to agents/ORG-CHART.md; docs/08-testing-guide.md carries the software verification matrix; README.md links agents/ORG-CHART.md under Architecture. Files: 32 new agent-org files + src/events/event-bus.ts + 3 docs updates. LOC added: ~3,000. LOC removed: ~2,800 (stale tests). Zero src/ behavioural changes. |
