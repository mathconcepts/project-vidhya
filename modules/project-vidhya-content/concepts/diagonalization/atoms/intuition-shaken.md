...
On $A=\begin{pmatrix}4&1\\2&3\end{pmatrix}$, the hook already found $P$ and $D$. Watch those same two numbers below.
...
Two shortcuts, no hunting needed: $n$ different eigenvalues, or $A$ symmetric ($A=A^T$). The full rule underneath: $n$ independent eigenvectors, one full set per eigenvalue. If an eigenvalue's independent eigenvectors (its *geometric multiplicity*) fall short of how many times it repeats as a root (its *algebraic multiplicity*), diagonalization fails.
