...
id: numerical-linear-algebra.worked_example.assured
...
LU without pivoting exists only when every leading principal minor of $A$ is nonzero. Here they are $2$, $(2)(3)-(1)(4)=2$, and $\det A=\det U=2\cdot1\cdot2=4$ — all nonzero, so the factorisation below needs no permutation.

$$L=\begin{pmatrix}1&0&0\\2&1&0\\4&3&1\end{pmatrix},\qquad U=\begin{pmatrix}2&1&1\\0&1&1\\0&0&2\end{pmatrix}$$

Forward substitution on $Ly=b$ gives $y=(4,2,2)^T$; back substitution on $Ux=y$ gives $x=(1,1,1)^T$ — both $O(n^2)$, cheap once $U$'s $O(n^3)$ factorisation is already paid for, which is exactly why LU beats repeating Gaussian elimination for every new right-hand side.
