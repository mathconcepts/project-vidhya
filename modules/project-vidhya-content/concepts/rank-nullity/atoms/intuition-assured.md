...
$\text{rank}(A)+\text{nullity}(A)=n$, $n$ = number of columns — every input dimension either survives into the output (rank) or collapses to zero (nullity), nothing left over.
