...
Take $f(z)=1/z$ on the circle $|z|=1$, counterclockwise. Parametrize $z=e^{it}$, $t\in[0,2\pi]$, $dz=ie^{it}dt$: $\oint_Cf(z)\,dz=\int_0^{2\pi}\frac{ie^{it}}{e^{it}}\,dt=\int_0^{2\pi}i\,dt=2\pi i$. The formula for any curve is exactly this, $\oint_Cf(z)\,dz=\int_a^bf(z(t))z'(t)\,dt$, worked out here for one specific one.
