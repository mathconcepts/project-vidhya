...
*Answer:* Exactly one, in all three cases. $\det A = -2 \neq 0$, so $A^{-1}$ exists, so $x = A^{-1}b$ is defined and unique **for every $b$**. Invertibility is a property of $A$ alone — it settles the solution count before you have even looked at $b$. This is the "iff" in *$A^{-1}$ exists $\iff$ $Ax=b$ has a unique solution for every $b$*.
...
Check in the original equations: $1 + 4 = 5$ ✓ and $3 + 8 = 11$ ✓.

If $\det A = 0$ the count is no longer determined by $A$ alone: the system has **either** no solution **or** infinitely many, and which one you get depends entirely on $b$ — specifically on whether $b$ lies in the column space of $A$. "Singular" never means "no solution"; it means "uniqueness is gone."

**Why this drill exists:** the misconception is that a singular matrix implies an inconsistent system. It doesn't — $\det A = 0$ kills *uniqueness*, not *existence*. Students carrying the wrong version confidently mark "no solution" on systems that in fact have infinitely many, and never think to check whether $b$ is in $\text{Col}(A)$.
