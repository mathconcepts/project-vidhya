...
- **`institution`** is rank 5 (above owner) and is **scaffolding** for the multi-tenant B2B tier (deferred). Default-disabled via the `auth.institution_role` feature flag. Until that flag is on and tenancy isolation lands, assigning the role is rejected by the user-store.
