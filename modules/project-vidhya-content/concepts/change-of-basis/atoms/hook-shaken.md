...
A point written $(3,2)$ is really an address on one grid — the standard $x$-$y$ grid.

Push the two axis arrows, $e_1$ and $e_2$, through the matrix. Watch where each one lands.

$e_1$ lands at $(1,-1)$. $e_2$ lands at $(1,2)$. Those two landing arrows are the new basis — read straight off the matrix's two columns.
