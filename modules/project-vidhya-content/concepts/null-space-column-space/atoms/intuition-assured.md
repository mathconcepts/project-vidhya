...
Same matrix as the scene above, $C=\begin{pmatrix}1&-1\\-1&1\end{pmatrix}$ — read as a definition rather than a demonstration.
...
$\text{Null}(C)=\text{span}(1,1)$, $\text{Col}(C)=\text{span}(1,-1)$ — kernel and image of the same map, generally living in different spaces ($\mathbb{R}^n$ vs $\mathbb{R}^m$ for an $m\times n$ matrix), here coinciding only because $C$ is square. Where it earns marks: $\text{Col}(A)=\mathbb{R}^m \iff A$ surjective; $\text{Null}(A)=\{0\} \iff A$ injective; both together, for square $A$, mean invertible.
