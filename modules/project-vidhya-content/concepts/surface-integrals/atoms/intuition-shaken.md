...
Take the uniform field $\mathbf F=(0,0,3)$ and a flat unit square $S$ lying in the $xy$-plane, $z=0$. The outward normal is $\hat n=\hat k$, so $\mathbf F\cdot\hat n=3$ everywhere on $S$, and since the field never changes, $\iint_S\mathbf F\cdot\hat n\,dS=3\times(\text{area }1)=3$.
