...
export function Button({
  variant = 'filled', tone = 'mastery', size = 'md', full = false,
  disabled = false, icon = null, iconAfter = null, onClick, children,
}) {
  ...
  const skin =
    variant === 'filled' ? { background: down ? t.press : t.solid, color: 'var(--text-on-accent)', border: 'none' }
    : variant === 'tinted' ? { background: t.tint, color: t.ink, border: 'none' }
    : variant === 'grey' ? { background: 'var(--surface-fill)', color: 'var(--text-primary)', border: 'none' }
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: s.radius, fontFamily: 'var(--font-sans)', fontSize: s.font,
        fontWeight: 'var(--weight-semibold)', letterSpacing: 'var(--tracking-body)',
