...
**What makes it "definite":** it isn't enough to see one closed ellipse — a saddle has closed-looking level curves too, near the center, for the wrong reason. The real signature is that *every* level curve closes, for any $c$. The animation on this card grows the level curves of a positive definite form ($x^2+4y^2=c$, the dark curve) alongside an indefinite form ($x^2-4y^2=c$, the lighter grey curve) on the same axes: the first family stays a bounded ellipse at every $c$, while the second opens into two unbounded hyperbola branches — the level curve of a saddle never closes.
