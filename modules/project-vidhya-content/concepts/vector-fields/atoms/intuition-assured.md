...
For $\phi=\tfrac12(x^2+y^2)$ — whose gradient $\nabla\phi=(x,y)$ is exactly the field $\mathbf F(x,y)=(x,y)$ from the hook — the level curves are circles $x^2+y^2=c$. At $(1,1)$ the circle's tangent runs in the $(-1,1)$ direction, while $\nabla\phi=(1,1)$ points radially outward — orthogonal to it, not along it. Reaching for the tangent direction when a question asks for the gradient is a $90°$ error, and it is the more tempting mistake precisely because both directions are "natural" ones to read off the same picture.
