...
$A=Q\Lambda Q^T$ is the orthogonal special case of diagonalization on the hook's own $A=\begin{pmatrix}2&2\\2&-1\end{pmatrix}$: same $PDP^{-1}$ shape, but $Q^{-1}=Q^T$, so inverting the change of basis costs nothing — watched below, not assumed. This is why $f(A)=Qf(\Lambda)Q^T$ generalizes to any function applied entrywise on $\Lambda$ — square root, exponential, arbitrary powers — without ever forming $Q^{-1}$ separately.
